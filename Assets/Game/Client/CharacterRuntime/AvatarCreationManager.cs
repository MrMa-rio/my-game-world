...
namespace MyGameWorld.Client.CharacterRuntime
{
    public readonly struct AvatarCreationRequest
    {
        ...
        { Seed = seed; Traits = traits; Parent = parent; Completed = completed; }
    ...
    public sealed class AvatarCreationManager : MonoBehaviour
    {
        ...
        {
            EnsureInitialized(); CharacterAppearanceDNA appearance = ResolveAppearance(seed, traits);
            RuntimeAvatar avatar = _pool.Count > 0 ? _pool.Pop() : new GameObject("Runtime Avatar").AddComponent<RuntimeAvatar>();
            avatar.transform.SetParent(parent, false); avatar.gameObject.SetActive(true); avatar.Initialize(appearance);
        ...
        private void Update()
        {
            ...
            for (int i = 0; i < count; i++) { AvatarCreationRequest request = _queue.Dequeue(); RuntimeAvatar avatar = CreateImmediately(request.Seed, request.Traits, request.Parent); request.Completed?.Invoke(avatar); }
        }
        ...
        private void MaterializePart(RuntimeAvatar avatar, CharacterPartSelection selection)
        {
            ...
            ApplyPalette(part, avatar.Appearance, selection.Slot); avatar.AddPart(part);
        }

        private static void ApplyPalette(GameObject part, CharacterAppearanceDNA appearance, CharacterPartSlot slot)
        {
