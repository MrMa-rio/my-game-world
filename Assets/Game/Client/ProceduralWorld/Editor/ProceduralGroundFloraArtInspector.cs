...
{
    public static class ProceduralGroundFloraArtInspector
    {
        ...
        [MenuItem("My Game World/Source Art/Capture Procedural Ground Flora Gallery")]
        public static void CaptureGallery()
        {
            ...
            GameObject host = new GameObject("Procedural Ground Flora Runtime Gallery");
            ...
            try
            {
                ZoneDNA zone = new ZoneDNA(new ZoneId(1), 829172, BiomeId.TemperateGrassland,
                    TerrainProfileId.RollingLowPoly, TerrainGeneratorV3.GeneratorVersion, new AssetCatalogVersion(2));
                DecorationKind[] kinds = { DecorationKind.Flower, DecorationKind.FlowerCluster, DecorationKind.Mushroom, DecorationKind.MushroomCluster };
                for (int index = 0; index < kinds.Length; index++)
                {
                    ...
                    DecorationPlacement definition = new DecorationPlacement(new WorldElementId(index + 1), zone,
                        1000 + index, kind, WorldVisualAssetIds.ForDecoration(kind),
                        new WorldVector3((index - 1.5f) * 2.2f, 0f, 0f), index * 19f, 1.25f);
                    manager.Request(new ProceduralGenerationRequest(definition,
                        new ProceduralEnvironmentContext(BiomeId.TemperateGrassland, Vector3.up, 0f, 0.8f),
                        ProceduralVisualLod.High, GenerationPriority.High));
                }
                manager.FlushQueue();

                Camera camera = new GameObject("Ground Flora Gallery Camera").AddComponent<Camera>();
                camera.clearFlags = CameraClearFlags.SolidColor;
                camera.backgroundColor = new Color(0.76f, 0.84f, 0.88f);
                camera.fieldOfView = 28f;
                camera.transform.position = new Vector3(0f, 1.05f, -10f);
                camera.transform.LookAt(new Vector3(0f, 0.48f, 0f));

                Light light = new GameObject("Ground Flora Gallery Key Light").AddComponent<Light>();
        ...
        private static void Capture(Camera camera)
        {
            ...
            try
            {
                ...
                string path = Path.GetFullPath("Logs/ProceduralGroundFloraGallery.png");
                Directory.CreateDirectory(Path.GetDirectoryName(path)); File.WriteAllBytes(path, capture.EncodeToPNG());
                Debug.Log($"Captured procedural ground flora gallery to {path}.");
            }
            finally
            {
