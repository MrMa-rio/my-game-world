...
{
    public sealed class ZoneGeneratorV4
    {
        ...
        public ZoneGeneratorV4(TerrainGenerationConfig config, BiomeDefinition biome, WorldGenerationLimits limits = null)
        {
        ...
        public ZoneGenerationResult Generate(ZoneDNA dna)
        {
            ...
            TerrainGenerationResult terrain = new TerrainGeneratorV4(_config, _biome, features).Generate(dna, context);
            var liquids = new LiquidBodyPlannerV1().Plan(dna, terrain, features);
            ZoneFeaturePlan completedFeatures = new ZoneFeaturePlan(features.Terrain, features.Landforms, features.Paths, liquids);
            return new ZoneGenerationResult(dna, terrain, new DecorationGeneratorV3(_limits).Generate(dna, terrain, _biome), completedFeatures);
        }
    }
}
