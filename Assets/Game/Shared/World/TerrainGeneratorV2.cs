...
{
    public sealed class TerrainGeneratorV2 : ProceduralGenerator<ZoneDNA, TerrainGenerationResult>
    {
        public static readonly GeneratorVersion GeneratorVersion = new GeneratorVersion(2);
        ...
        public TerrainGeneratorV2(TerrainGenerationConfig config, BiomeDefinition biome, ZoneFeaturePlan features) : base(GeneratorVersion)
        {
        ...
        protected override GenerationValidation ValidateCore(ZoneDNA dna, GenerationContext context)
        {
            ...
            if (dna.TerrainProfileId != TerrainProfileId.RollingLowPoly) return GenerationValidation.Invalid("TERRAIN_PROFILE_UNSUPPORTED", "TerrainGeneratorV2 supports RollingLowPoly only.");
        ...
        protected override TerrainGenerationResult GenerateCore(ZoneDNA dna, GenerationContext context)
        {
            ...
            float[] heights = new float[resolution * resolution]; float[] paths = new float[heights.Length];
            float spacingX = _config.Width / (resolution - 1); float spacingZ = _config.Depth / (resolution - 1);
            for (int z = 0; z < resolution; z++)
            for (int x = 0; x < resolution; x++)
            {
                float worldX = (-_config.Width * 0.5f) + (x * spacingX); float worldZ = (-_config.Depth * 0.5f) + (z * spacingZ);
                HeightSample sample = heightGenerator.Sample(worldX, worldZ); int index = z * resolution + x;
                heights[index] = sample.Height; paths[index] = sample.PathMask;
            }
