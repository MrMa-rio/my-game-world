...
{
    public sealed class TerrainGeneratorV6 : ProceduralGenerator<ZoneDNA, TerrainGenerationResult>
    {
        public static readonly GeneratorVersion GeneratorVersion = new GeneratorVersion(6);
        private readonly TerrainGenerationConfig _config; private readonly BiomeDefinition _biome;
        private readonly ZoneFeaturePlan _features; private readonly LargeScaleTerrainProfile _profile;
        private readonly TerrainMeshDataBuilder _meshBuilder = new TerrainMeshDataBuilder();
        public TerrainGeneratorV6(TerrainGenerationConfig config, BiomeDefinition biome, ZoneFeaturePlan features, LargeScaleTerrainProfile profile) : base(GeneratorVersion)
        { _config = config ?? throw new ArgumentNullException(nameof(config)); _biome = biome ?? throw new ArgumentNullException(nameof(biome)); _features = features ?? throw new ArgumentNullException(nameof(features)); _profile = profile ?? throw new ArgumentNullException(nameof(profile)); }
        ...
        {
            HeightFieldGeneratorV2 source = new HeightFieldGeneratorV2(dna, _config, _biome, _features, _profile);
